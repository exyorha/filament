...
wgpu::PipelineLayout const& WebGPUPipelineLayoutCache::getOrCreatePipelineLayout(
        ...
        const size_t bindGroupLayoutCount) {
    PipelineLayoutKey key{};
    populateKey(bindGroupLayouts, bindGroupLayoutCount, key);
    ...
    const wgpu::PipelineLayout layout{ createPipelineLayout(label, bindGroupLayouts,
...
void WebGPUPipelineLayoutCache::populateKey(
        std::array<wgpu::BindGroupLayout, MAX_DESCRIPTOR_SET_COUNT> const& bindGroupLayouts,
        const size_t bindGroupLayoutCount, PipelineLayoutKey& outKey) {
    outKey.bindGroupLayoutCount = static_cast<uint8_t>(bindGroupLayoutCount);
    for (size_t bindGroupIndex{ 0 }; bindGroupIndex < bindGroupLayoutCount; ++bindGroupIndex) {
        outKey.bindGroupLayoutHandles[bindGroupIndex] = bindGroupLayouts[bindGroupIndex].Get();
    }
}

wgpu::PipelineLayout WebGPUPipelineLayoutCache::createPipelineLayout(utils::CString const& label,
        std::array<wgpu::BindGroupLayout, MAX_DESCRIPTOR_SET_COUNT> const& bindGroupLayouts,
        const size_t bindGroupLayoutCount) {
    ...
    FILAMENT_CHECK_POSTCONDITION(layout)
            << "Failed to create pipeline layout " << label.c_str_safe() << "?";
    return layout;
}
